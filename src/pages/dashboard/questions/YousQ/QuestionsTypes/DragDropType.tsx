...
export default function DragDropType({
  topicIndex,
  questionIndex,
  control,
  errors,
  ...
  const { fields, append, remove } = useFieldArray({
    control,
    name: `Topics.${topicIndex}.questions.${questionIndex}.Answers_Drop`,
  });
  const rootAnswerError = getNestedValue(
    errors,
    `Topics.${topicIndex}.questions.${questionIndex}.Answers_Drop`
  );
  return (
    <div className="grid grid-cols-4 gap-4">
      ...
      <FormController
        ...
        name={`Topics.${topicIndex}.questions.${questionIndex}.ContentQuestion_Drop`}
        control={control}
        ...
        {fields.map((answer, index) => (
          <div key={answer.id} className="grid grid-cols-7 gap-4">
            <FormController
              ...
              name={`Topics.${topicIndex}.questions.${questionIndex}.Answers_Drop.${index}.Answer`}
              control={control}
            ...
            <FormController
              label="Order"
              name={`Topics.${topicIndex}.questions.${questionIndex}.Answers_Drop.${index}.CorrectAnswerOrder`}
              control={control}
